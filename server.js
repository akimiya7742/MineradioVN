...
async function handleSearch(keywords, limit = 20) {
    ...
    // Use --flat-playlist for speed (only gets metadata, doesn't probe formats)
    const args = `--dump-single-json --flat-playlist --playlist-items 1-${limit} "ytsearch${limit}:${keywords}"`;
    ...
    return entries.map(item => ({
        ...
        url: item.url // Note: this is the web URL, not the stream URL
    }));
}
...
async function handleGetDetail(id) {
    const args = `--dump-json "https://www.youtube.com/watch?v=${id}"`;
    const output = await runYtDlp(args);
...
server.listen(PORT, HOST, () => {
  ...
  console.log(` Mode: yt-dlp wrapper`);
  console.log(`======================================================`);
});
